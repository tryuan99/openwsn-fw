...
static void channel_cal_average_scum_tx_tuning_codes_for_channel(void) {
    ...
    for (uint8_t i = 0;
        ...
        while (
            i < g_channel_cal_scum_tx_num_received_tuning_codes_for_channel -
                    1 &&
...
static inline void channel_cal_print_received_scum_tx_tuning_code(void) {
    snprintf(
        g_channel_cal_uart_buffer, UART_TX_MAX_LENGTH,
        "%c%02d %02d %02d %02d\n", g_channel_cal_rx_packet_crc ? '+' : '-',
        g_channel_cal_channel,
        ...
        switch (g_channel_cal_state) {
            ...
            case CHANNEL_CAL_STATE_TX: {
                ...
                memset(&g_channel_cal_tx_packet, 0,
                       sizeof(channel_cal_rx_packet_t));
                ...
                for (uint8_t i = 0;
                     i < CHANNEL_CAL_MAX_NUM_TX_TUNING_CODES_PER_CHANNEL; ++i) {
                    g_channel_cal_tx_packet
                        .tx_tuning_codes[i] = g_channel_cal_scum_tx_tuning_codes
                        [g_channel_cal_channel - CHANNEL_CAL_MIN_CHANNEL][i];
                }

...
uint8_t _uart_rxByte_openWSN(void);
void 	_uart_rxByte(uint8_t);
...
void hdlc_init(hdlc_rx_cbt hdlc_rx_cb) {
	...
	uart_setCallbacks(_uart_txByteDone, _uart_rxByte_openWSN);
...
void hdlc_tx(uint8_t* buf, uint8_t bufLen) {
    ...
    // start TX'ing
    uart_writeByte(hdlc_vars.txBuf[hdlc_vars.txBufIdxR++]);
}
